...
for (c in sort(unique(as.character(dataset$country)))) {
  ...
  if (c %in% c("Diamond Princess", "Cruise Ship", "MS Zaandam", "West Bank and Gaza")) {
    next
  }
...
# check if all countries but the excluded ones have been found
...
date_validation_start <- "2020-03-26"
...
data.table::fwrite(
  submit,
   paste0(outdir, "week3-submission.csv"))
